    ...
    where TContext : DbContext
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (entities is null || entities.Count == 0)
            throw new Exception("Entities is not found!");
    ...
    public async Task<TEntity?> GetByIdAsync(TId id, bool enableTracking = true, bool include = true,
        CancellationToken cancellationToken = default)
    {
        if (id is null)
        ...
        if (include is false)
            query = query.IgnoreAutoIncludes();
        return await query.FirstOrDefaultAsync(e => e.Id != null && e.Id.Equals(id), cancellationToken);
    }

    public async Task<int> CountAsync(Expression<Func<TEntity, bool>>? filter = null, bool include = true,
        CancellationToken cancellationToken = default)
    {
        IQueryable<TEntity> query = context.Set<TEntity>();
        if (include is false)
